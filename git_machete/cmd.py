...
def advance(b):
    ...
    if ans in ('y', 'yes'):
        ...
        down_branches[b] = flat_map(
            ...
            down_branches[b])
        save_definition_file()
        run_post_slide_out_hook(b, d, down_branches.get(d) or [])
        ...
        "hooks": """
            ...
            * <b>machete-post-slide-out <new-upstream> <lowest-slid-out-branch> [<new-downstreams>...]</b>
                ...
                * <b><new-downstreams></b> are all the following (possibly zero) parameters,
                  ...
                  Note that this is guaranteed to be exactly one branch in case of `slide-out` (but no guarantees exist in case of `advance` and `traverse`).

                The hook, if present, is executed:
                * zero or one times during each `advance` execution, depending on whether user confirms the slide-out operation
                  (and not e.g. just stops after fast-forwarding the current branch),
                * exactly once during each successful `slide-out` execution (even if multiple branches are slid out),
                * possibly multiple times during each `traverse` execution (every time a slide-out operation is confirmed).

                If the hook returns a non-zero exit code, then the execution of the command is aborted:
                * `slide-out` won't attempt rebase of the new downstream branch
                * `traverse` won't continue the traversal.
                The behavior of `advance` (other than the exit code of the entire command) doesn't change since a slide-out is the last operation of `advance` anyway.
...
def launch(orig_args):
    ...
    try:
        ...
        if cmd != "help":
            ...
            if cmd != "discover" and not os.path.isfile(definition_file_path):
